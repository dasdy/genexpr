...
from genast.nodes import NumberNode, MultNode, FuncallNode, PowerNode, SumNode, SumSub, SignNode
...
def rand_sum_chain(amount, min, max):
    nodes = [MultNode(PowerNode(FuncallNode(None, rand_num_node(min, max)))) for _ in range(amount)]
    if amount > 1:
